...
```
Nous avons besoin de votre avis d’expert sur ce document. Il s’agit d’une vieille lettre et il semblerait que celle-ci soit importante pour le groupe de pirates que nous recherchons. Votre mission consiste à déchiffrer ce texte et à nous donner le nom complet de l’auteur ("Prénom Nom").
```
...
```
# Utiliser decode avec un déchiffrement automatique
# On remarque que la clé est THE MENTOR
# cherhcer sur google : THE MENTOR MANIFEST
# Nom + prénom : Loyd Blankenship
```